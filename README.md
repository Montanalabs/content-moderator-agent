...
Untrusted input is a post; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a post
- **Closed type:** `type Decision = ModPost(ModAction) | RemoveMod` where `ModAction` is closed
- **Sink / capability:** `grant moderatePost confidence 70` → `privileged { moderatePost(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/content-moderator-agent/demo.sh
```
...
- `content-moderator-agent_safe.wave` — the correct design.
- `content-moderator-agent_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
